    ...
    <div class="content-wrapper pb-0">
        <div class="page-header">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('categories') }}">Kateqoriyalar</a></li>
                </ol>
            </nav>
        </div>
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" value="{{ $category->name }}" name="name" class="form-control" placeholder="Ad">
                @error('name')
            ...
            <div class="form-group">
                ...
                <input type="text" value="{{ $category->slug }}" name="slug" class="form-control" placeholder="Slug">
                @error('slug')
                    <span class="text-danger mt-2">{{ $message }}</span>
                @enderror
            </div>
            <div class="form-group">
                ...
                <textarea name="description" id="editor" class="form-control">{{ $category->description }}</textarea>
                @error('description')
                    <span class="text-danger mt-2">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                @if ($category->icon)
                    <div class="upload-box">
                        <div class="image-box">
                            <img src="{{ asset('uploads/icons/' . $category->icon) }}" width="50" alt="">
                        </div>
                    </div>
                @endif
            </div>
